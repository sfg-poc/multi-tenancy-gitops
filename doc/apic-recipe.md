...
1. Make sure the `storageClassName` specified in `${GITOPS_PROFILE}/2-services/argocd/instances/ibm-apic-instance.yaml` refers to a block storage class. **IMPORTANT:** IBM API Connect **requires block storage!**. By default, the storage class used for this recipe is **`ocs-storagecluster-cephfs`**. If you are using this recipe and the GitOps Framework that executes this recipe on another environment, make sure you select the appropriate storage class.
